<?php
    ...
    // 파일 업로드 처리    // 파일명이 한글일 경우 file_path에 아무것도 담기지 않고, 파일 업로드 또한 되지 않음.
    ...
    if ($_FILES['myfile']['error'] === UPLOAD_ERR_OK) {
        ...
        // 파일 업로드 실패한 경우도 고려하여 여기에 처리할 내용 추가는 나중의 내가 할겨.
    }

    $query = $connect->prepare("INSERT INTO freeBoard (title, content, userID, date, hit, thumb, file_name, file_path) 
    VALUES (?, ?, ?, ?, 0, 0, ?, ?)");
